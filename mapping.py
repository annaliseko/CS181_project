...
data = pd.read_csv('retweets.csv', keep_default_na=False)
data.loc = np.where(data.location.isnull(), data.timezone, data.location)
# loc_array = [x for x in loc_array if x != '']
# tz_array = data.timezone
# tz_array = [x for x in tz_array if x != '']
with open('output.csv', 'wb') as f:
    writer = csv.writer(f)
    writer.writerow(["user", "location", "timezone", "text"])
